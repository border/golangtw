...
func parseTokenAndSecret(data string) (string, string, os.Error) {
    ...
    fmt.Println("oauth.go response body: ", parts)
...
func (c *Consumer) httpExecute(method string, urlStr string, body string, oauthParams *OrderedParams) (*http.Response, os.Error) {
    ...
    fmt.Println("oauth.go request:", req)

    resp, err := c.HttpClient.Do(&req)
    fmt.Println("oauth.go response:", resp)
    ...
    if resp.StatusCode != http.StatusOK {
        ...
        resp.Body.Close()
        fmt.Println("oauth.go response body:", string(bytes))
